CLASS ltcl_scramble_book_dfkkesr DEFINITION FINAL FOR TESTING
  DURATION SHORT
  RISK LEVEL HARMLESS.
...
CLASS ltcl_scramble_book_dfkkesr IMPLEMENTATION.

  METHOD all_fields_are_scrambled.
    " Implicit Check in DB operator method UPDATE_DB
    " Relevant fields:
    " - NAME1, NAME2, POST_CODE1, CITY1, STREET, HOUSE_NUM1
    TRY.
  ...
  METHOD create_cut4one_field_scram.
    result = zcl_vcd_scramble_book_process=>create(
          hash_algo_values = VALUE #( hashsalt           = 'zUiFlkWj'
                                      ...
                                      codepage           = '1252'
                                      structure_name     = 'DFKKESR'
                                      fields_range_table = VALUE #( ( sign = 'I' option = 'EQ' low = 'NAME1' ) )
                                      package_size       = 200
                                      opbel_range_table  = VALUE #( )
                                      budat_range_table  = VALUE #( ) )
          appl_log         = NEW zvcd_cl_anwendungs_log( i_object        = 'ZVCD_REPORTS'
